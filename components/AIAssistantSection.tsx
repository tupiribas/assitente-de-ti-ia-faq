  ...
  const handleFAQProposal = (aiResponseText: string): boolean => {
    ...
    if (match && match[1]) {
      try {
        ...
        // REMOÇÃO DAS LINHAS DE ESCAPAMENTO DE BARRA INVERTIDA SE EXISTIREM.
        // rawJsonString = rawJsonString.replace(/\\/g, '\\\\'); // REMOVA ESTA LINHA SE EXISTIR

        // Substitui backticks por aspas duplas e aspas simples por aspas duplas (seguro).
        rawJsonString = rawJsonString.replace(/`/g, '');
        rawJsonString = rawJsonString.replace(/'/g, '"');

        // NOVO: Substitui a string 'undefined' por 'null' para JSON válido.
        // Isso é crucial para corrigir o 'Unexpected token u'.
