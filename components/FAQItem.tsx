...
//         <div className="flex items-center space-x-2"> {/* NOVO: Wrapper para o ícone de anexo e o chevron */}
//           {/* Verifica se o FAQ tem attachments (imageUrl ou documentUrl) */}
//           {!faq.imageUrl && !faq.documentUrl && ( // NOVO: Se não tiver imagem nem documento
//             <NoImageIcon className="w-5 h-5 text-slate-400" aria-label="Sem anexos visuais" />
//           )}
...
const FAQItem: React.FC<FAQItemProps> = ({ faq, isExpanded, onToggle, onEdit, onDelete }) => {
  const renderFaqAnswer = (answerContent: string) => {
    // Verifica se a resposta parece ser HTML (por exemplo, de um editor Quill)
    const looksLikeHtml = /<\/?[a-z][\s\S]*>/i.test(answerContent);

    if (looksLikeHtml) {
      // Limpa e renderiza HTML
      const cleanHtml = DOMPurify.sanitize(answerContent, {
      ...
      // Renderiza Markdown
      return <ReactMarkdown components={{ a: LinkRenderer }} remarkPlugins={[remarkGfm]}>{answerContent}</ReactMarkdown>;
    }
  };
