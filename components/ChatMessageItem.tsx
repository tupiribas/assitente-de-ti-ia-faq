  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3`}>
      <div className={`flex items-end max-w-xs md:max-w-md lg:max-w-lg ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        {isUser ? (
          <UserCircleIcon className={`w-8 h-8 rounded-full ${isUser ? 'ml-2 text-blue-500' : 'mr-2 text-purple-500'}`} />
        ) : (
          <CpuChipIcon className={`w-8 h-8 rounded-full ${isUser ? 'ml-2 text-blue-500' : 'mr-2 text-purple-500'}`} />
        )}
        <div
          className={`px-4 py-3 rounded-xl shadow ${
            isUser ? 'bg-blue-500 text-white rounded-br-none' : 'bg-slate-200 text-slate-800 rounded-bl-none'
          }`}
        >
          <div className="prose prose-sm max-w-none text-sm" dangerouslySetInnerHTML={formatText(message.text)} />
          <p className={`text-xs mt-1 ${isUser ? 'text-blue-200 text-right' : 'text-slate-500 text-left'}`}>
