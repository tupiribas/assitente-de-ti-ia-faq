...
// NOVO: Função para verificar se a hora atual está dentro do horário de funcionamento
const isServiceTime = () => {
  const now = new Date();
  ...
  // Certifique-se de que o servidor Fly.io está no fuso horário correto ou ajuste
  // Exemplo: De 7h00 às 12h00 e de 13h11 às 18h00
  const isMorning = (hour >= 7 && hour < 12); // Das 07:00:00 até 11:59:59
  const isAfternoon = (hour > 13 || (hour === 13 && minute >= 11)) && hour < 18; // Das 13:11:00 até 17:59:59

  // Para testar, você pode comentar essas linhas e retornar true
  return isMorning || isAfternoon;
};

// NOVO: Exemplo de Rate Limiting Básico em Memória
const requestCounts = new Map(); // Map<ip, count>
