...
// Inicializa o cliente GoogleGenAI globalmente
...
const storageChatImage = multer.memoryStorage(); // Armazena o arquivo em buffer na memória
const uploadChatImage = multer({ storage: storageChatImage }); //


// Middlewares
app.use(cors()); //
app.use(express.json()); // Middleware para JSON (para mensagens de texto e dados de FAQ)
...
const MAX_REQUESTS_PER_HOUR = 50; // Exemplo: 50 requisições por hora
const RESET_INTERVAL_MS = 60 * 60 * 1000; // 1 hora

setInterval(() => {
    requestCounts.clear(); // Reseta os contadores a cada hora
...
// MODIFICADO: Agora aceita multipart/form-data para imagem
app.post('/api/ai-chat', uploadChatImage.single('image'), async (req, res) => { //
    ...
    try {
        ...
        // ... (logs de depuração de imagem recebida) ...
        ...
        if (imageFile) {
            ...
            // NOVO: Normalizar o mimeType para formatos padrão aceitos pelo Gemini
            let normalizedMimeType = imageFile.mimetype;
            // Se for image/jpg, converte para image/jpeg (muito comum)
            if (normalizedMimeType === 'image/jpg') {
                normalizedMimeType = 'image/jpeg';
            }
            // Você pode adicionar mais normalizações aqui se encontrar outros problemas (ex: image/tiff -> image/jpeg se conversível)
            // No entanto, é importante que o formato da imagem seja realmente o do mimetype (ex: não mandar um PNG com mimetype JPEG)

            contentParts.push({
                inlineData: {
                    data: imageBase64,
                    mimeType: normalizedMimeType, // <--- Usar o mimeType normalizado
                },
            });
        }
        ...
        // ... (restante da inicialização da chatSession e sendMessage) ...
        ...
        const result = await chatSession.sendMessage({ parts: contentParts });
